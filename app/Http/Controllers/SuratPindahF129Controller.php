...
class SuratPindahF129Controller extends Controller
{
    /**
     * Generate default nomor surat F.1-29.
     */
    private function generateNomorSurat()
    {
        $romanMonths = [1=>'I', 2=>'II', 3=>'III', 4=>'IV', 5=>'V', 6=>'VI', 7=>'VII', 8=>'VIII', 9=>'IX', 10=>'X', 11=>'XI', 12=>'XII'];
        $monthRoman = $romanMonths[(int)date('n')];
        $count = SuratPindahF129::whereYear('created_at', date('Y'))->count() + 1;
        return sprintf("475/LMG-F129/%s/%03d/%d", $monthRoman, $count, date('Y'));
    }

    /**
     * Display form for creating Formulir Permohonan Pindah WNI (F.1-29).
     */
    public function create()
    {
        $nomorSuratDefault = $this->generateNomorSurat();
        return view('surat.f129-create', compact('nomorSuratDefault'));
    }

    /**
     * Store new permohonan pindah F.1-29 data.
     */
    public function store(Request $request)
    {
        ...
        return redirect()->route('surat.f129.print', $surat->id)
            ->with('success', 'Data berhasil disimpan! Dokumen siap dicetak.');
    }

    /**
     * Show printable document template for F.1-29.
     */
    public function print($id)
    {
        $surat = SuratPindahF129::findOrFail($id);
        return view('surat.f129-print', compact('surat'));
    }
}
