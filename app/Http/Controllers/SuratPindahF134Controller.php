...
class SuratPindahF134Controller extends Controller
{
    /**
     * Generate default nomor surat F.1-34.
     */
    private function generateNomorSurat()
    {
        $romanMonths = [1=>'I', 2=>'II', 3=>'III', 4=>'IV', 5=>'V', 6=>'VI', 7=>'VII', 8=>'VIII', 9=>'IX', 10=>'X', 11=>'XI', 12=>'XII'];
        $monthRoman = $romanMonths[(int)date('n')];
        $count = SuratPindahF134::whereYear('created_at', date('Y'))->count() + 1;
        return sprintf("475/LMG-F134/%s/%03d/%d", $monthRoman, $count, date('Y'));
    }

    /**
     * Display form for creating Formulir Permohonan Pindah WNI Antar Kabupaten/Kota atau Antar Provinsi (F.1-34).
     */
    public function create()
    {
        $nomorSuratDefault = $this->generateNomorSurat();
        return view('surat.f134-create', compact('nomorSuratDefault'));
    }

    /**
     * Store new permohonan pindah F.1-34 data.
     */
    public function store(Request $request)
    {
        ...
        return redirect()->route('surat.f134.print', $surat->id)
            ->with('success', 'Data berhasil disimpan! Dokumen F.1-34 siap dicetak.');
    }

    /**
     * Show printable document template for F.1-34.
     */
    public function print($id)
    {
        $surat = SuratPindahF134::findOrFail($id);
        return view('surat.f134-print', compact('surat'));
    }
}
